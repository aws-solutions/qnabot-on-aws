...
describe('import component', () => {
    ...
    test('questionHasErrors', () => {
        ...
        expect(wrapper.vm.questionHasErrors(wrapper.vm, noQidQuestion, 1)).toBe(true);
        expect(wrapper.vm.$data.errorList.length).toEqual(1);
        expect(wrapper.vm.$data.errorList[0]).toEqual(`Warning: No QID found for line ${1}. The question will be skipped.`);

        expect(wrapper.vm.questionHasErrors(wrapper.vm, noQuestionQuestion, 1)).toBe(true);
        expect(wrapper.vm.$data.errorList.length).toEqual(2);
        expect(wrapper.vm.$data.errorList[1]).toEqual(`Warning: No questions found for QID: "${noQuestionQuestion.qid}". The question will be skipped.`);

        expect(wrapper.vm.questionHasErrors(wrapper.vm, noAnswerQuestion, 1)).toBe(true);
        expect(wrapper.vm.$data.errorList.length).toEqual(3);
        expect(wrapper.vm.$data.errorList[2]).toEqual(`Warning: No answer found for QID:"${noQuestionQuestion.qid}". The question will be skipped.`);

        expect(wrapper.vm.questionHasErrors(wrapper.vm, validQuestion, 1)).toBe(false);
        expect(wrapper.vm.$data.errorList.length).toEqual(3);
    });
