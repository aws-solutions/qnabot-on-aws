...
describe('when calling step function', () => {
    ...
    it('should successfully execute step', async () => {
        jest.spyOn(qnabotSettings, 'merge_default_and_custom_settings').mockResolvedValue({ EMBEDDINGS_ENABLE: false });
        ...
        s3Mock.on(GetObjectCommand).resolvesOnce({ Body: sdkStream1, ContentRange: 'bytes 0-1299/1300' }).resolvesOnce({ Body: sdkStream2, ContentRange: 'bytes 0-1299/1300' }); 
        ...
        expect(s3Mock).toHaveReceivedNthCommandWith(2, GetObjectCommand, {"Bucket": "qna-test-importbucket", "Key": "data/import_questions.json"});
        expect(s3Mock).toHaveReceivedNthCommandWith(3, GetObjectCommand, {"Bucket": "qna-test-importbucket", "Key": "data/import_questions.json", "Range": "bytes=0-20000", "VersionId": "testVersion"});
    ...
    it('should handle an error with second GetObjectCommand', async () => {
        ...
        expect(s3Mock).toHaveReceivedCommandTimes(GetObjectCommand, 2);
        expect(s3Mock).toHaveReceivedNthCommandWith(2, GetObjectCommand, {"Bucket": "qna-test-importbucket", "Key": "data/import_questions.json"});
        expect(s3Mock).toHaveReceivedNthCommandWith(3, GetObjectCommand,  {"Bucket": undefined, "Key": undefined, "Range": "bytes=undefined-undefined", "VersionId": undefined});
        expect(s3Mock).toHaveReceivedCommandTimes(PutObjectCommand, 1);
        expect(s3Mock).toHaveReceivedCommandWith(PutObjectCommand, {"Body": "{\"status\":\"Error\",\"message\":\"{}\"}", "Bucket": "qna-test-importbucket", "Key": "data/import_questions.json"});
    ...
    it('should handle an error with buffer', async () => {
        ...
        s3Mock.on(GetObjectCommand).resolvesOnce({ Body: sdkStream1}).resolvesOnce({ Body: sdkStream2, ContentRange: 'bytes 0-2525/2526' });
