...
const { ComprehendClient, DetectDominantLanguageCommand, DetectSentimentCommand } = require('@aws-sdk/client-comprehend');
...
describe('parse function with Lex event', () => {
    ...
    test("when parsing request with sentimemt & multilang support disabled", async () => {
        settings.merge_default_and_custom_settings.mockReturnValue(parseFixtures.defaultSettings);
        const res = {};
        const parseResponse = await parse(parseFixtures.createRequestObject("What is QnABot?", "Text", null, null), res);
    ...
    test("when parsing request with multilang support enabled", async () => {
        ...
        settings.merge_default_and_custom_settings.mockReturnValue(parseFixtures.defaultSettingsMultiLang);
    ...
    test("when parsing request with sentiment support enabled", async () => {
        ...
        settings.get_parameter.mockReturnValue("https://cognito-idp.us-east-1.amazonaws.com/us-east-1dsfsfjl");
        settings.merge_default_and_custom_settings.mockReturnValue(parseFixtures.defaultSettingsSentiment);
...
describe('parse function with Alexa event', () => {
    ...
    test("should be able to call parse function", async () => {
        ...
        settings.merge_default_and_custom_settings.mockReturnValue(parseFixtures.defaultSettings);
        const parseResponse = await parse(parseFixtures.createRequestObject("What is QnABot?", "Text", "version", { "locale": "en-US" }), {})
        expect(parseResponse.req).toBeDefined()
        expect(parseResponse.req._type).toBe("ALEXA")
        expect(parseResponse.req._clientType).toBe("ALEXA")
        expect(parseResponse.req._settings.DEFAULT_USER_POOL_JWKS_URL).toEqual("https://cognito-idp.us-east-1.amazonaws.com/us-east-1dsfsfjl");
    });
