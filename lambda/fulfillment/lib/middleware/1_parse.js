...
function getClientType(req) {
    ...
    if ((_.get(req, '_event.requestAttributes.x-amz-lex:channel-type') == 'Twilio-SMS') || (_.get(req, '_event.requestAttributes.x-amz-lex:channels:platform') == 'Twilio')) {
...
module.exports = async function parse(req, res) {
    ...
    Object.assign(res, {
        ...
        message: '',
        session: _.mapValues(
            _.omit(_.cloneDeep(req.session), ['appContext']),
            (x) => {
                try {
                    return JSON.parse(x);
                } catch (e) {
