...
async function get_userLanguages(inputText) {
    ...
    const languages = await comprehendClient.detectDominantLanguage(params).promise();
    return languages;
}
...
function set_userLocale(Languages, userPreferredLocale, defaultConfidenceScore, req) {
    ...
    } else if (req.session.userDetectedSecondaryLocale) {
        delete req.session.userDetectedSecondaryLocale;
    }
