...
function getMarkdownMessage(answerMessageMd, answerMessage, event, answerDocumentUris, foundAnswerCount, seenTop, helpfulDocumentsUris, maxDocumentCount) {  // NOSONAR Need all params
    ...
    if (answerDocumentUris.size > 0) {
        ...
        answerDocumentUris.forEach((element) => {
            ...
            if (signS3Urls) {
                signS3URL(element.DocumentURI, expireSeconds, (url) => {
                    element.DocumentURI = url;
                });
            }
    ...
    helpfulDocumentsUris.forEach((element) => {
        if (idx++ < maxDocumentCount) {
            ...
            // Convert S3 Object URLs to signed URLs
            if (signS3Urls) {
                signS3URL(element.uri, expireSeconds, (url) => {
                    element.uri = url;
                });
            }
