...
@click.option("-d", "--delete-existing-content", type=click.BOOL, help="Use this parameter if all existing QnABot {qids} in your QnABot deployment should be deleted before the import process.", required=False, default=False, show_default=True)
@click.pass_context
def qna_import (ctx, cloudformation_stack_name: str, source_filename: str, file_format: str, delete_existing_content: bool):
    ...
    """
    try: 
        response = qnabot_cli_helper.initiate_import (cloudformation_stack_name = cloudformation_stack_name, source_filename = source_filename, file_format = file_format, delete_existing_content = delete_existing_content) #proceed with upload file to Amazon S3
        click.echo (response)
        sys.exit (0)
    except OSError as e: 
...
def qna_export (ctx, cloudformation_stack_name: str, export_filename: str, export_filter: str, file_format: str):
    ...
    """
    try: 
        response = qnabot_cli_helper.initiate_export (cloudformation_stack_name = cloudformation_stack_name, export_filename = export_filename, export_filter = export_filter, file_format = file_format) #proceed with initiating the export process
        click.echo (response)
        sys.exit (0)
    except OSError as e: 
        qnabot_cli_helper.error_response (
