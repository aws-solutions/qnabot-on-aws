BUCKET=$(shell bin/exports.js | jq --raw-output '."QNA-BOOTSTRAP-BUCKET"')
PREFIX=$(shell node -e "console.log(JSON.stringify(require('./config')))" | jq --raw-output '."publicPrefix"')
